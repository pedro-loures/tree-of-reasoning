...
def test_split_payload_to_shards(tmp_path: Path):
    payload = {
        ...
        "runs": [{"tree_key": "capitals:test:brazil:plain:0:0", "country_id": "brazil"}],
        "trees": {
    ...
    assert "/abs/" not in manifest["source"]
...
def test_build_tutorial_payload_picks_smallest():
    payload = {
        "runs": [
            {"tree_key": "big", "summary": {"total_leaves": 100}, "prompt": "big", "model_id": "m", "tau": 0.01},
            {"tree_key": "small", "summary": {"total_leaves": 2}, "prompt": "small", "model_id": "m", "tau": 0.01},
        ],
        "trees": {
            "big": [{"id": "root"}],
            "small": [{"id": "root"}],
