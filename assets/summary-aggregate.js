...
function emptyBucket() {
  return {
    ...
    nodes_green: 0, nodes_blue: 0, nodes_grey: 0, nodes_yellow: 0, nodes_red: 0,
  };
}
...
function addToBucket(bucket, row) {
  ...
  for (const color of ["green", "blue", "grey", "yellow", "red"]) {
    const key = `nodes_${color}`;
    if (row[key] != null) bucket[key] += row[key];
  }
  const motifHits = row.motif_hits_by_length || {};
  for (const [length, hits] of Object.entries(motifHits)) {
    bucket.motif_hits_by_length[length] = (bucket.motif_hits_by_length[length] || 0) + (Number(hits) || 0);
  }
}
...
function finalizeBucket(key, label, bucket) {
  ...
  const row = {
    ...
    avg_reasoning_tokens: round2(bucket.reasoning_tokens / trees),
    avg_mean_entropy: bucket._entropy_n ? round2(bucket.mean_entropy / bucket._entropy_n) : null,
    avg_mean_logprob: bucket._logprob_n ? round2(bucket.mean_logprob / bucket._logprob_n) : null,
    avg_exclusively_bad: badN ? round2(bucket.exclusively_bad / badN) : null,
    avg_nodes_green: round2(bucket.nodes_green / trees),
    avg_nodes_blue: round2(bucket.nodes_blue / trees),
    avg_nodes_grey: round2(bucket.nodes_grey / trees),
    avg_nodes_yellow: round2(bucket.nodes_yellow / trees),
    avg_nodes_red: round2(bucket.nodes_red / trees),
    ...
    nodes_ditched_after: bucket._expansion_n
      ? round2(bucket.nodes_ditched_after / bucket._expansion_n) : null,
    avg_tau_star: bucket._tau_star_n ? round2(bucket.tau_star_sum / bucket._tau_star_n) : null,
    avg_candidates: badN ? round2(bucket.candidates / badN) : null,
    ...
    motif_hits_by_length: { ...bucket.motif_hits_by_length },
  };
  for (const [length, hits] of Object.entries(bucket.motif_hits_by_length || {})) {
    row[TD.motifLenKey(length)] = hits;
  }
  return row;
}
