...
TD.SummaryPanel = class {
  ...
  renderTable(rows, onRowClick) {
    ...
    thead.innerHTML = TD.buildSummaryTableHead(this.columns, "whole");
    ...
    tbody.innerHTML = sorted.map(row => {
      const cells = this.columns.map(col => `<td data-key="${col.key}" class="${col.highlight ? "highlight" : ""}${col.freezeDefault ? " frozen" : ""}" style="text-align:${col.align || "right"}">${utils.escapeHtml(String(utils.fmtVal(row[col.key])))}</td>`).join("");
      return `<tr class="clickable" data-tree-key="${utils.escapeHtml(row.tree_key || "")}">${cells}</tr>`;
    }).join("");
    TD.applyFrozenColumns(this.tableEl, new Set(
      this.columns.filter(col => col.freezeDefault).map(col => col.key),
    ));
