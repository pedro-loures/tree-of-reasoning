...
(function (TD) {
const {
  DataStore, TreeGraph, SummaryPanel, switchSubTab, utils,
  aggregateSummaries, renderBarChart, createCapitalsBadFilters, renderDetailPanel,
  ColorSequencesPanel,
} = TD;

const SURGEON_TABLE_COLUMNS = [
  ...
  { key: "avg_nodes_green", label: "Green", chartable: true },
  { key: "avg_nodes_blue", label: "Blue", chartable: true },
  { key: "avg_nodes_grey", label: "Grey", chartable: true },
  { key: "avg_nodes_yellow", label: "Yellow", chartable: true },
  { key: "avg_nodes_red", label: "Red", chartable: true },
...
const SURGEON_ROW_COLUMNS = [
  { key: "country_name", label: "Model" },
  { key: "prefix_length", label: "Prefix" },
  { key: "max_depth", label: "Depth" },
  { key: "total_nodes", label: "Nodes" },
  { key: "leaf_count", label: "τ-leaves" },
  { key: "nodes_green", label: "Green" },
  { key: "nodes_red", label: "Red" },
  { key: "prob_good_pct", label: "P(good) %", highlight: true },
  { key: "prob_bad_pct", label: "P(bad) %", highlight: true },
];

TD.SurgeonAdapter = class {
  constructor(root) {
    this.root = root;
    this.id = name => `surgeon${name}`;
    this.el = name => root.querySelector(`#${this.id(name)}`);
    this.store = new DataStore("surgeon", "data/surgeon");
    this.graph = new TreeGraph(`#${this.id("Graph")}`, `#${this.id("Tooltip")}`);
    this.summary = new SummaryPanel(root, SURGEON_ROW_COLUMNS);
    this.summary.tableEl = this.el("SummaryTable");
    this.summary.statsEl = this.el("GlobalStats");
    this.summary.hintEl = this.el("FilterHint");
    this.badFilters = createCapitalsBadFilters("plain");
    this.sequences = new ColorSequencesPanel(document.getElementById("surgeonPanel_sequencesPanel"), {
      prefix: "surgeonSeq",
      store: this.store,
      fileField: "color_sequences_file",
      defaultFile: "color_sequences.json",
      countLabel: "endings",
      windowLabel: "nodes",
    });
    this.extended = new ColorSequencesPanel(document.getElementById("surgeonPanel_extendedPanel"), {
      prefix: "surgeonExt",
      store: this.store,
      fileField: "extended_sequences_file",
      defaultFile: "extended_sequences.json",
      countLabel: "leaves",
      windowLabel: "leaves",
    });
  ...
  async init() {
    try {
      ...
      this.sequences.store = this.store;
      this.extended.store = this.store;
      this.populateFilters();
      this.renderSummary();
      this.bindEvents();
      this.updateSnapshotBanner();
      ...
      if (source) {
        source.textContent =
          `Surgeon riddle · ${this.store.manifest.viewer_runs || 0} trees · generated ${this.store.manifest.generated_at}`;
      }
    } catch (err) {
      const source = this.el("Source");
  ...
  aggRows() {
    ...
    return aggregateSummaries(this.filteredSummaries(), dimension).map(row => ({
      ...row,
      label: row.label,
      avg_nodes_green: row.avg_nodes_green,
      avg_nodes_blue: row.avg_nodes_blue,
      avg_nodes_grey: row.avg_nodes_grey,
      avg_nodes_yellow: row.avg_nodes_yellow,
      avg_nodes_red: row.avg_nodes_red,
    }));
  }
  ...
  summaryColumns() {
    return TD.tableColumnsWithMotifs(SURGEON_TABLE_COLUMNS, this.filteredSummaries(), { chartable: true });
  }

  populateFilters() {
    const summaries = this.store.DATA.tree_summaries || [];
    const models = [...new Set(summaries.map(r => r.country_id).filter(Boolean))];
    this.el("ModelFilter").innerHTML =
      '<option value="">All models</option>' + models.map(c => {
    ...
    const tableColumns = TD.tableColumnsWithMotifs(SURGEON_TABLE_COLUMNS, summaries, { chartable: true });
  ...
  renderSummary() {
    ...
    if (hint) {
      hint.textContent = `${this.filteredSummaries().length} trees · ${aggRows.length} groups`;
    }
  ...
  updateSnapshotBanner() {
    const banner = this.el("SnapshotBanner");
    if (!banner) return;
    const tau = this.store.manifest.snapshot_min_path_prob;
    const summaries = this.store.DATA?.tree_summaries || [];
    if (!tau || !summaries.length) {
      banner.hidden = true;
      return;
    }
    const original = summaries.reduce((sum, row) => sum + (row.original_total_nodes || row.total_nodes || 0), 0);
    const snapshot = summaries.reduce((sum, row) => sum + (row.snapshot_total_nodes || 0), 0);
    banner.hidden = false;
    banner.innerHTML =
      `<strong>Path-probability snapshot.</strong> Tree Explorer shows nodes with path prob ≥ ${tau} only `
      + `(${snapshot.toLocaleString()} published nodes vs ${original.toLocaleString()} in the full τ-tree). `
      + `Summary tables and sequence views use the full tree.`;
  }
  ...
  refreshStatsForMode() {
    ...
    const minProb = parseFloat(this.el("ProbFilter")?.value || "0.01");
  ...
  renderBadNodeList(nodes, statsMap, leafMap) {
    ...
    el.querySelectorAll("[data-node]").forEach(div => {
      div.addEventListener("click", () => {
        ...
        this.graph.focusNode(nodeId, nodes, parseFloat(this.el("ProbFilter")?.value || "0.01"));
  ...
  async loadRun(treeKey) {
    ...
    if (meta && summary) {
      const orig = summary.original_total_nodes || summary.total_nodes;
      const snap = summary.snapshot_total_nodes || nodes.length;
      meta.textContent = `${summary.country_name || summary.country_id} · prefix ${summary.prefix_length} · snapshot ${snap.toLocaleString()} / ${orig.toLocaleString()} nodes`;
    }
    this.renderBadNodeList(nodes, statsMap, leafMap);
    this.renderRunStats(leafMap);
  ...
  bindEvents() {
    this.root.querySelectorAll(".tab-btn").forEach(btn => {
      btn.addEventListener("click", async () => {
        switchSubTab(this.root, btn.dataset.tab);
        if (btn.dataset.tab === "trees") await this.ensureTreeLoaded();
        if (btn.dataset.tab === "sequences") await this.sequences.show();
        if (btn.dataset.tab === "extended") await this.extended.show();
      });
    });
    ["ModelFilter", "PrefixFilter", "AggSelect", "MetricSelect"].forEach(name => {
      this.el(name)?.addEventListener("change", () => this.renderSummary());
    });
...
document.addEventListener("experiment-tab", event => {
  if (event.detail.experiment !== "surgeon" || window._surgeonInit) return;
  try {
    const adapter = new TD.SurgeonAdapter(document.getElementById("surgeonPanel"));
    window._surgeonInit = true;
    adapter.init();
  } catch (err) {
    const el = document.getElementById("surgeonSource");
