#!/usr/bin/env python3
"""Build capitals dashboard with tau-star expansion overlays."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build capitals expanded bad-nodes dashboard")
    parser.add_argument(
        "--mech-dir",
        type=Path,
        default=ROOT.parent / "tree-of-reasoning" / "results" / "capitals_mech_interp",
        help="Stage-1 tau-tree JSONL directory",
    )
    parser.add_argument(
        "--bad-nodes-dir",
        type=Path,
        default=ROOT.parent / "tree-of-reasoning" / "results" / "capitals_bad_nodes",
        help="Stage-2 baseline bad-nodes JSONL directory",
    )
    parser.add_argument(
        "--expanded-dir",
        type=Path,
        default=ROOT.parent / "tree-of-reasoning" / "results" / "capitals_bad_nodes_expanded",
        help="Stage-3 expanded bad-nodes JSONL directory",
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "output" / "capitals_expanded",
    ...
    expanded_path = args.expanded_dir / f"{args.model}.jsonl"
    ...
    print(f"Expanded trees: {payload['expanded_bad_nodes_trees']}")
    ...
    print(f"  http://localhost:8768/output/capitals_expanded/capitals_dashboard.html")
    return 0
